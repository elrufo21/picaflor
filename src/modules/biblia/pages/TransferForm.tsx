...
export default function TransferForm() {
  ...
  const updateField = useCallback(
    <K extends keyof TravelPackageFormState>(
      key: K,
  ...
  const setPaxCount = (value: string) => {
    const count = Math.min(
      99,
      Math.max(0, Number(value.replace(/\D/g, "")) || 0),
    );
    setForm((previous) => ({
      ...previous,
  ...
  useEffect(() => {
    setDays((previous) =>
      buildTransferDays(form.fechaInicioViaje, form.fechaFinViaje, previous),
    );
  ...
  const totals = useMemo(() => {
    const pasajeros = form.pasajeros.reduce(
      (sum, passenger) => sum + roundCurrency(passenger.totalTipoPasajero),
      0,
    );
    const servicios = days.reduce(
      (sum, day) => sum + roundCurrency(day.precio),
      0,
    );
    return {
      pasajeros,
      servicios,
      total: roundCurrency(pasajeros + servicios),
    };
  }, [days, form.pasajeros]);
  ...
  const submit = async () => {
    if (
      ...
      !form.programa.trim() ||
      !days.length ||
      !form.pasajeros.length
    ) {
      showToast({
        title: "Completa el traslado",
        description: "Indica agencia, programa, fechas y pasajeros.",
        type: "warning",
      });
      return;
    }
    if (
      form.pasajeros.some((passenger) => !passenger.nombres.trim()) ||
      days.some(
        (day) =>
          !day.destino.trim() ||
          !day.hora ||
          !Number.isFinite(Number(day.precio)) ||
          Number(day.precio) < 0,
      )
    ) {
      showToast({
        ...
        description:
          "Completa el pasajero, destino, hora y precio de cada día.",
        type: "warning",
      });
      return;
    }
    ...
    try {
      ...
      const id = await saveTransfer({
        ...
        counterId: usuarioId,
        agenciaId:
          Number.isInteger(agenciaId) && agenciaId > 0 ? agenciaId : null,
      ...
      showToast({
        ...
        description: `Registro #${id} creado.`,
        type: "success",
      });
  ...
  return (
    <div className="mx-auto max-w-8xl rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:p-5">
      <header className="sticky top-2 z-30 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            onClick={() => navigate(-1)}
            aria-label="Volver"
            className="rounded-lg p-1 text-slate-700 hover:bg-white"
          >
            <ChevronLeft size={22} />
          </button>
          ...
          <span className="text-sm text-slate-600">
            Fecha emisión: {form.fechaEmision}
          </span>
        </div>
      ...
      <main className="mt-4 space-y-2">
        <SectionCard
          icon={CarFront}
          title="1. Fecha y programa"
          description="Selecciona las fechas para crear un servicio por día."
        >
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_120px_150px]">
            ...
            <TextField
              label="Programa"
              value={form.programa}
              onChange={(event) => updateField("programa", event.target.value)}
              size="small"
              fullWidth
              inputProps={{
                autoComplete: "off",
        ...
        <PassengersSection
          ...
          onRemove={(id) =>
            setForm((previous) => ({
              ...previous,
              pasajeros: previous.pasajeros.filter(
        ...
        <SectionCard
          icon={CarFront}
          title="5. Totales"
          description="Resumen del traslado."
        >
          <div className="ml-auto max-w-sm overflow-hidden rounded-xl border border-slate-200 text-sm">
            <div className="flex justify-between border-b border-slate-200 px-4 py-2">
              <span>Pasajeros</span>
              <strong>{money(totals.pasajeros)}</strong>
            </div>
            <div className="flex justify-between border-b border-slate-200 px-4 py-2">
              <span>Servicios</span>
              <strong>{money(totals.servicios)}</strong>
            </div>
            <div className="flex justify-between bg-amber-100 px-4 py-3 font-semibold text-amber-950">
