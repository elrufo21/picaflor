...
const LIQUIDACION_FIELDS = [
  ...
  { key: "comercialUsuarioId", label: "ComercialUsuarioId", sourceIndex: 55 },
  { key: "comercialNombre", label: "ComercialNombre", sourceIndex: 56 },
] as const;
...
const LiquidacionesPage = () => {
  ...
  const handleView = async (row: LiquidacionRow) => {
    ...
    const normalizedData = {
      ...
      acuenta: parseMoneyValue(data.acuenta),
      saldo: parseMoneyValue(data.saldo),
      precioTotal: parseMoneyValue(data.totalPagar),
      totalGeneral: parseMoneyValue(data.totalPagar),

      entidadBancaria: data.entidadBancaria,
      nroOperacion: data.nroOperacion,
      efectivo: parseMoneyValue(data.efectivo),
      deposito: parseMoneyValue(data.deposito),

      documentoCobranza: data.notaDocu,

      precioExtra: parseMoneyValue(data.adicional),
