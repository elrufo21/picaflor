...
const TravelPackageBoletaPreview = () => {
  ...
  const boletaFilename = useMemo(() => {
    ...
    const idLabel = String(boletaData?.notaId ?? id ?? "SIN-ID").trim();
    return `${fecha}_PAQ_ID_${idLabel}.pdf`;
  }, [boletaData, id]);
          ...
          isMobile ? (
            <div className="flex flex-col items-center gap-3 rounded-xl border border-slate-200 p-6 text-center">
              <p className="text-sm text-slate-600">
                La vista previa no esta disponible en movil.
              </p>
              <button
                type="button"
...
export default TravelPackageBoletaPreview;
