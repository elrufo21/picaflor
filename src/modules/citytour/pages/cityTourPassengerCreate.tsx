...
import { PackageHeader } from "../components/create-passenger/CityTourHeader";
...
const PackagePassengerCreate = () => {
    ...
    const loadDetails = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/Programacion/traer-actividades/${liquidacionId}`,
          {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/programacion/agregar-viaje`,
        {
          method: "POST",
